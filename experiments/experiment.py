...
class ApproxExperiment:
    """
    Helper class to track intermediate states of an experiment

    This abstracts handling/storing/restoring of intermediate model states during an experiment,
    i.e. check if a baseline has already been trained in the past and only create a new one if none is present
    this speeds up subsequent execution and improves repeatability of sub-experiments.
    """
    ...
    def quantized_model(self, qconfig: quant.QConfig, qtype: str) -> ApproxNet:
        ...
        quant_path = os.path.join(
            self.model_dir, f"{self.model_fp32.name.lower()}_{qtype.lower()}.pt"
        )
        quant_model.qconfig = qconfig

        if not os.path.exists(quant_path):
            logger.debug(
                f"No quantized model for mode {qtype} found in {quant_path}. Training a new one."
            )
            pl.seed_everything(42)
            quant_model.train_baseline_quant(
                self.datamodule, test=self.test, mlf_params={"qtype": qtype}
            )
